...
class Download {
    ...
    public function get_the_ID(){
        return get_option('academyc_default_certificate_id');
    }
	...
	public function get_image_id() {		
		...
		if ( $this->get_the_ID() ) {
			$image_ids = get_post_meta( $this->get_the_ID(), '_image_ids', true );
	...
	public function generate_pdf() {
		...
		// Get Course Data
		$course               = array();
		$course['post_title'] = $post->post_title;
		$course_end_date      = date( 'Y-m-d' );

		// Get the certificate template
		$certificate_template_custom_fields = get_post_custom( $this->get_the_ID() );
		...
		$date = \AcademyCertificates\Helper::get_certificate_formatted_date( $course_end_date );
		...
		ConfigTFPDF::output_to_http(
			$fpdf, $current_user->display_name . '-certificate.pdf'
		);
